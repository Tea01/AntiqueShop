...
{
    public partial class sikke : Form
    {
        public sikke()
        {
            InitializeComponent();
        ...
        private void sikke_Load(object sender, EventArgs e)
        {
            string sorgu = "select * from urun where kategori_turu='sikke'";          //bej kushtin qe te marri vtm nga mobilyet
            NpgsqlDataAdapter da = new NpgsqlDataAdapter(sorgu, conn);
            DataSet ds = new DataSet();
            da.Fill(ds);
            dataGrid_sikke.DataSource = ds.Tables[0];
        }
