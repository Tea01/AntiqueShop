...
{
    public partial class vinylPlaka : Form
    {
        public vinylPlaka()
        {
            InitializeComponent();
        ...
        private void vinylPlaka_Load(object sender, EventArgs e)
        {
            string sorgu = "select * from urun where kategori_turu='vinyl plak'";          //bej kushtin qe te marri vtm nga mobilyet
            NpgsqlDataAdapter da = new NpgsqlDataAdapter(sorgu, conn);
            DataSet ds = new DataSet();
            da.Fill(ds);
            dataGrid_vinylPlaka.DataSource = ds.Tables[0];
        }
